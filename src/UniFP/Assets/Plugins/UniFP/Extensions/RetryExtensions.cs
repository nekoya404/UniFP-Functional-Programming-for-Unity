using System;
using System.Threading;
using Cysharp.Threading.Tasks;

namespace UniFP
{
    /// <summary>
    /// Retry and repeat utilities for Result operations
    /// </summary>
    public static class RetryExtensions
    {
        #region Retry Sync

        /// <summary>Retry operation with delay</summary>
        public static Result<T> Retry<T>(
            Func<Result<T>> operation,
            int maxAttempts,
            int delayMilliseconds = 0)
        {
            if (maxAttempts <= 0)
                throw new ArgumentException("maxAttempts must be greater than 0", nameof(maxAttempts));
            if (operation == null)
                throw new ArgumentNullException(nameof(operation));

            Result<T> lastResult = default;

            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                lastResult = operation();

                if (lastResult.IsSuccess)
                    return lastResult;

                if (attempt < maxAttempts - 1 && delayMilliseconds > 0)
                    Thread.Sleep(delayMilliseconds);
            }

            return lastResult;
        }

        /// <summary>Retry until condition is met</summary>
        public static Result<T> RetryUntil<T>(
            Func<Result<T>> operation,
            Func<Result<T>, bool> continueCondition,
            int maxAttempts,
            int delayMilliseconds = 0)
        {
            if (maxAttempts <= 0)
                throw new ArgumentException("maxAttempts must be greater than 0", nameof(maxAttempts));
            if (operation == null)
                throw new ArgumentNullException(nameof(operation));
            if (continueCondition == null)
                throw new ArgumentNullException(nameof(continueCondition));

            Result<T> result = default;

            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                result = operation();

                if (!continueCondition(result))
                    return result;

                if (attempt < maxAttempts - 1 && delayMilliseconds > 0)
                    Thread.Sleep(delayMilliseconds);
            }

            return result;
        }

        #endregion

        #region Retry Async

        /// <summary>Retry async operation with delay</summary>
        public static async UniTask<Result<T>> RetryAsync<T>(
            Func<UniTask<Result<T>>> operation,
            int maxAttempts,
            int delayMilliseconds = 0)
        {
            if (maxAttempts <= 0)
                throw new ArgumentException("maxAttempts must be greater than 0", nameof(maxAttempts));
            if (operation == null)
                throw new ArgumentNullException(nameof(operation));

            Result<T> lastResult = default;

            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                lastResult = await operation();

                if (lastResult.IsSuccess)
                    return lastResult;

                if (attempt < maxAttempts - 1 && delayMilliseconds > 0)
                    await UniTask.Delay(delayMilliseconds);
            }

            return lastResult;
        }

        /// <summary>Retry async operation with exponential backoff</summary>
        public static async UniTask<Result<T>> RetryWithBackoff<T>(
            Func<UniTask<Result<T>>> operation,
            int maxAttempts,
            int initialDelayMilliseconds = 100,
            float backoffMultiplier = 2.0f)
        {
            if (maxAttempts <= 0)
                throw new ArgumentException("maxAttempts must be greater than 0", nameof(maxAttempts));
            if (operation == null)
                throw new ArgumentNullException(nameof(operation));

            Result<T> lastResult = default;
            int delay = initialDelayMilliseconds;

            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                lastResult = await operation();

                if (lastResult.IsSuccess)
                    return lastResult;

                if (attempt < maxAttempts - 1)
                {
                    await UniTask.Delay(delay);
                    delay = (int)(delay * backoffMultiplier);
                }
            }

            return lastResult;
        }

        #endregion

        #region Repeat

        /// <summary>Repeat operation N times</summary>
        public static Result<T> Repeat<T>(
            Func<Result<T>> operation,
            int count)
        {
            if (count <= 0)
                throw new ArgumentException("count must be greater than 0", nameof(count));
            if (operation == null)
                throw new ArgumentNullException(nameof(operation));

            Result<T> result = default;

            for (int i = 0; i < count; i++)
            {
                result = operation();
                if (result.IsFailure)
                    return result;
            }

            return result;
        }

        /// <summary>Repeat async operation N times</summary>
        public static async UniTask<Result<T>> RepeatAsync<T>(
            Func<UniTask<Result<T>>> operation,
            int count)
        {
            if (count <= 0)
                throw new ArgumentException("count must be greater than 0", nameof(count));
            if (operation == null)
                throw new ArgumentNullException(nameof(operation));

            Result<T> result = default;

            for (int i = 0; i < count; i++)
            {
                result = await operation();
                if (result.IsFailure)
                    return result;
            }

            return result;
        }

        #endregion
    }
}
